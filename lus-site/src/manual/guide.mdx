...
icon: compass
group: learn
shortdesc: "The full language reference."
---
...
The `<const>` attribute makes a variable immutable after initialization, so any attempt to reassign it is a compile-time error:
...
Groups are not first-class values, so they cannot be passed to functions or stored in tables. You also cannot add new fields after initialization. However, you can perform partial reassignment to update only specific fields:
...
String literals come in several forms. Single-quoted (`'hello'`) and double-quoted (`"hello"`) strings support escape sequences: `\n` (newline), `\t` (tab), `\\` (backslash), `\"`, `\'`, `\xNN` (hex byte), `\u{NNNN}` (Unicode codepoint), and `\z` (skip following whitespace). Long strings use `[[...]]` or `[=[...]=]` (with matching equals signs) and preserve their content verbatim:
...
If the value before `?` is falsy, the entire remaining chain is skipped and that falsy value is returned. `false` is preserved. Multiple `?` operators can appear in a single chain. A common pattern combines optional chaining with `or` for fallbacks:
...
For tables, `#t` returns the length of the _sequence_. If the array portion has holes (nil values between entries), the result is not well-defined. Custom behavior can be defined with the `__len` metamethod.
...
Multiple assignments work the same way, so the branch is taken only when every assigned value is truthy: `if a, b = getTwoValues() then`.
...
Only `return f(args)` is a proper tail call, so wrapping the call in parentheses or adding operations after it breaks the optimization.
...
Removing entries never shrinks a table's storage. After clearing a large table, [`table.compact`](/manual/api#table.compact) releases the unused capacity.
...
A stateless iterator relies only on the invariant state and the control variable, so no closures or mutable state. This is the most efficient form:
...
Patterns are _not_ regular expressions, so they lack alternation (`|`), backreferences, and other regex features. However, they handle most practical tasks:
...
For security-critical sandboxing, the `pledge` function provides fine-grained capability-based access control. Permissions follow a hierarchical naming scheme, so granting a base permission implicitly grants all sub-permissions:
...
[`require`](/manual/api#require) loads and caches modules. On the first call, it searches for the module, runs it, and stores the return value in [`package.loaded`](/manual/api#package.loaded). Subsequent calls return the cached value:
...
`require("name")` searches for files using the patterns in [`package.path`](/manual/api#package.path) (for Lus files) and [`package.cpath`](/manual/api#package.cpath) (for C libraries). The `?` placeholder is replaced with the module name:
...
The [`pledge`](/manual/api#pledge) function (covered in [Sandboxing](#sandboxing)) controls access to sensitive operations. All libraries below that perform I/O require appropriate permissions. Available permission categories include `fs`, `fs:read`, `fs:write`, `network`, `network:http`, `network:tcp`, `network:udp`, `exec`, `env`, and `load`. The `io` library is gated by the `fs` permissions, and [`os.getenv`](/manual/api#os.getenv) by `env`.
...
The [`fromcsv`](/manual/api#fromcsv) and [`tocsv`](/manual/api#tocsv) globals provide the same round trip for CSV data.

`tojson` accepts an optional filter function as a second argument for transforming or omitting fields during serialization. Tables with a `__json` metamethod (see [Other metamethods](#other-metamethods)) can customize their JSON representation. Special float values ([`math.huge`](/manual/api#math.huge), `-math.huge`, NaN) serialize as `null`; circular references produce an error.
...
Key functions include `fs.type(path)` (returns `"file"` or `"directory"` and whether it is a symlink), `fs.remove(path, recursive)`, and the [`fs.path`](/manual/api#fs.path) utilities ([`fs.path.join`](/manual/api#fs.path.join), [`fs.path.split`](/manual/api#fs.path.split), [`fs.path.name`](/manual/api#fs.path.name), [`fs.path.parent`](/manual/api#fs.path.parent)).
...
`network.fetch(url, method, headers, body)` performs HTTP requests and returns the status code, response body, and response headers. TCP sockets support `send`, `receive` (with patterns: a number for N bytes, `"*l"` for a line, `"*a"` for all available data), `settimeout`, and `close`. UDP sockets are available via [`network.udp.open`](/manual/api#network.udp.open).

### Workers

[Workers](/manual/api#worker) are separate interpreter states running on a thread pool, providing true concurrency through message passing. Arguments passed to [`worker.create`](/manual/api#worker.create) are deep-copied into the worker's state:
...
Inside a worker, `worker.message(data)` sends data back to the parent, and `worker.peek()` blocks until a message arrives. [`worker.receive`](/manual/api#worker.receive) can accept multiple workers and returns from whichever has a message first (select-like). Workers inherit the parent's permissions in sealed form, so a worker can never hold more permissions than its parent. Creating a worker requires the `load` and `fs:read` permissions to read its script file.
...
The Lus interpreter is a register-based virtual machine where the operands of each instruction either represents a position on the machine's stack or a constant within the currently executing function. Each instruction within the Lus instruction set maps closely to a language construct; `OP_SLICE` refers to the slice syntax (`t[i,j]`), `OP_ADD` refers to additive arithmetic (`a + b`), and so on. Calls to known standard-library functions compile to a dedicated fast-dispatch instruction that executes them inline; running with `--readonly-env` freezes `_ENV` and the module tables, letting that dispatch skip its runtime validation.

### Memory management

Lus implements an incremental mark-and-sweep garbage collector with an optional generational mode. Objects allocated by the runtime are tracked by the collector; as allocations accumulate, it interleaves short marking and sweeping steps with program execution, reclaiming everything no longer reachable from the program's roots.

The garbage collector can also operate _generationally_, meaning it can divide objects into generations based on their age. As most objects in programs die young, we can account for their rapid expiration and optimize accordingly. Newly created objects belong to the young generation and are collected frequently; objects that survive multiple collection cycles are promoted to the old generation, which is collected less often. The standalone interpreter runs in generational mode by default; embedders start in incremental mode.

You rarely need to think about garbage collection in practice. However, if you're working with performance-sensitive code or managing large amounts of data, Lus exposes the [`collectgarbage`](/manual/api#collectgarbage) function to let you interface with the garbage collector. Two interpreter flags also help memory-constrained deployments: `--gc-pause N` bounds how far the heap may grow past the live set (in percent) before an incremental collection cycle starts, trading CPU for a smaller peak footprint, and `--strip-debug` drops debug information from loaded code at the cost of source detail in tracebacks.
