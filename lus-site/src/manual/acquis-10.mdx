...
acquis: true
shortdesc: "A pledge-like capability-based permissions system for explicitly defining what your code needs access to."
---
...
| `env`          | Reading environment variables (`os.getenv`)                       |
...
> The `loadfile`, `dofile`, and `require` functions require both `fs:read` (to read the file) and `load` (to execute the code). The `load` function requires only `load`. Loading native modules (`package.loadlib`, `require` of C modules) also requires both. The `io` library is gated by the `fs` permissions: `io.open` and friends need `fs:read`/`fs:write` as appropriate, and `io.tmpfile` and `os.tmpname` need `fs:write`.
